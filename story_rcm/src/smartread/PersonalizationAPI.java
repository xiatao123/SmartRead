...
public class PersonalizationAPI {
    private static final Logger logger = LogManager
            .getLogger(PersonalizationAPI.class);
    ...
    public static void main(String args[]) throws InterruptedException {
        ...
        case 5:
        case 10:
            api.updateUserInterestsRaw(lookbackMinute);
            ...
            break;
        case 60:
            api.updateUserInterests("1h");
            break;
        case 60 * 24:
            api.updateUserInterests("1d");
            break;
        case 60 * 24 * 7:
            api.updateUserInterests("7d");
            break;
        default:
    ...
    private List<Story> calcualte(User user, List<Story> stories) {
        ...
        logger.debug("Time(ms) taken to calculate story point for user "
                + user.getUid() + ": " + String.valueOf(endtime - starttime));
        return stories;
    }
    ...
    private void timelineFactor(List<Story> stories) {
        ...
        for (Story s : stories) {
            ...
            double factor = 1 - (date.getTime() - sDate.getTime()) * 0.05
                    / 1000 / 60 / 60 / 24;
            logger.trace("Timeline factor for story " + s.getStoryID() + "is "
                    + factor);
            s.setBScore(s.getBScore() * factor);
        }
        Long endtime = System.currentTimeMillis();
        logger.debug("Time(ms) taken to update story score based on timeline: "
                + String.valueOf(endtime - starttime));
    }

    /*
     * public List<Story> getUserStory(String uid) { Long starttime =
     * System.currentTimeMillis(); List<Story> stories =
     * DBStory.retrieveDefaultStory(); User user = DBUser.retrieveUser(uid);
     * Long endtime = System.currentTimeMillis();
     * 
     * logger.debug("Time(ms) taken to retrive user story: "+
     * String.valueOf(endtime-starttime));
     * 
    ...
    public Set<String> updateUserInterestsRaw(int lookbackMinute) {
        ...
        Map<String, List<ServeEvent>> serves = DBServeEvent
                .QueryEvents(lookbackMinute);
        ...
        Long endtime = System.currentTimeMillis();
        logger.debug("Time(ms) taken to update user interests for "
                + lookbackMinute + " minutes: "
                + String.valueOf(endtime - starttime));
    ...
    private void updateUserStoryForAll() {
        ...
        Long endtime = System.currentTimeMillis();
        logger.debug("Time(ms) taken to refresh all users' story score: "
                + String.valueOf(endtime - starttime));
    }
}